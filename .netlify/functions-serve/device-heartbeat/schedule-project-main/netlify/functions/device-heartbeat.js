...
var device_heartbeat_exports = {};
__export(device_heartbeat_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(device_heartbeat_exports);
...
// ../netlify/functions/device-heartbeat.js
var sql = src_default(process.env.NEON_DATABASE_URL, {
  ssl: "require"
});
  ...
  try {
    if (event.httpMethod === "POST") {
      ...
      if (body.action === "update_meta") {
        const { deviceId: deviceId2, friendlyName, isPinned } = body;
        if (!deviceId2) {
          return { statusCode: 400, headers, body: JSON.stringify({ error: "Device ID required" }) };
        }
        const updateData = {};
        if (friendlyName !== void 0) updateData.friendly_name = friendlyName;
        if (isPinned !== void 0) updateData.is_pinned = isPinned;
        if (Object.keys(updateData).length > 0) {
          await sql`
                        UPDATE device_status 
                        SET ${sql(updateData)}
                        WHERE device_id = ${deviceId2}
                    `;
        }
        return {
          statusCode: 200,
      ...
      if (body.action === "delete") {
        const { deviceId: deviceId2 } = body;
        if (!deviceId2) {
          return { statusCode: 400, headers, body: JSON.stringify({ error: "Device ID required" }) };
        }
        await sql`
                    DELETE FROM device_status WHERE device_id = ${deviceId2}
                `;
        return {
          statusCode: 200,
          headers,
...
//# sourceMappingURL=device-heartbeat.js.map
