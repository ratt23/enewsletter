...
// ../netlify/functions/api.js
var sql = src_default(process.env.NEON_DATABASE_URL, { ssl: "require" });
  ...
  try {
    if (path.startsWith("/doctors")) {
      ...
      if ((path === "/doctors" || path === "/doctors/") && method === "GET") {
        ...
        const searchFilter = search ? sql`WHERE name ILIKE ${"%" + search + "%"} OR specialty ILIKE ${"%" + search + "%"}` : sql``;
          ...
          if (method === "PUT") {
            ...
            const [updated] = await sql`
                            UPDATE mcu_packages SET 
                                ...
                                addons=${addons ? JSON.stringify(addons) : null}, is_enabled=${is_enabled !== void 0 ? is_enabled : true}, 
    ...
    if (error.status) return { statusCode: error.status, headers, body: JSON.stringify({ message: error.message }) };
